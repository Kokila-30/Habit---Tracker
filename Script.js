...
for(let row=0; row<rows; row++)
{
    ...
    input.addEventListener("input",()=>{data[`habit_${row}`] = input.value;
        saveData();
    });
...
for(let i=1; i<=5; i++)
    ...
    goalCheck.addEventListener("change",()=>{data[`goalCheck_${i}`] = goalCheck.checked;
        saveData();
    });
...
sleepLevels.forEach(level=>
    ...
    for(let day=1; day<=31; day++)
        ...
        dot.addEventListener("click",()=>{ data[`sleep_${day}`] = level;
            saveData();
            refreshSleepGraph();
        });
        td.appendChild(dot);
...
function showDailyReminder(){
    ...
    if(
        ...
        new Notification(
            ...
            {
                body:
                "It's 8:30 PM! Don't forget to update today's habits 😊"
            }
        );
    }
}
